"""
Oakwood Home & Garden — mock data and tool functions.

This is the original domain. Data extracted from tools.py to match
the domain pack structure (each domain has data.py, prompts.py, scenarios.py).
"""
...
def issue_refund(order_id: str, amount: float, reason: str, guardrails_enabled: bool = True) -> str:
    """Issue a refund for an order. Hard guardrail: amounts over £50 are blocked."""
    amount = float(amount)
    order = ORDERS.get(order_id.upper())
    if not order:
...
DOMAIN_META = {
    ...
    "tagline": "UK garden retailer — customer service agent",
    "description": (
        "Oakwood Home & Garden is a fictional UK garden retailer selling tools, seeds, "
        "BBQs, fire pits, and outdoor furniture online. They have 10 recent orders "
        "and category-specific refund policies."
    ),
